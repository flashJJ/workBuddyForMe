import { assistantUpdateSchema, idParamSchema } from '@wbfm/shared';
...
export const GET = defineRoute(({ params, services }) => {
  ...
  return jsonOk(services.assistants.get(id));
});

export const PATCH = defineRoute(async ({ request, params, services }) => {
  const { id } = parseParams(idParamSchema, params);
  const input = parseBody(assistantUpdateSchema, await readJsonBody(request));
  return jsonOk(services.assistants.update(id, input));
});

export const DELETE = defineRoute(({ params, services }) => {
  const { id } = parseParams(idParamSchema, params);
  services.assistants.delete(id);
  return jsonOk({ id });
});

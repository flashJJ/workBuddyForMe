...
"架构债"这个词由 Martin Fowler 在 2003 年提出——类似金融债，你借的是"现在不还钱，但以后要还利息"。v0.1 到 v0.2 这一年多里，WorkBuddy 积累了不少架构债：有的是 v0.1 设计时就预留的扩展点但一直没实现，有的是 v0.2 开发期间为了快速交付做的临时方案，有的是外部依赖的升级跟不上导致的类型摩擦。
...
Ollama 的 `/v1/chat/completions` 流式模式不返回 usage（非流式返回）。WorkBuddy 当前处理方式是把 usage 设为 null，前端 UI 跳过显示。但如果要做对话成本统计（v0.3 规划的功能），usage 是必要输入。
...
| langsmith SDK 没有引入，自己写了 trace client | SDK 200KB+ 重依赖，WorkBuddy 只用了 startRun/end 两个 API |
...
架构债盘点的核心不是"还清所有债"，而是**对每一笔债都有清晰的优先级和还债窗口**。WorkBuddy 的架构从 v0.1 到 v0.2 保持了健康的演进——没有引入无法维护的技术债务，大部分债都是"知道可以更好但暂时不影响功能"的温和债。这归功于三层工程门禁（TS strict + 300 行 + 外部调用 mock）守住了代码质量的底线。

## 本系列到此结束

B01-B10 覆盖了 WorkBuddy v0.2 从架构升级、工具调用完整实现、SSE 事件编排、Ollama 集成、冷启动优化、LangSmith 追踪零侵入、消息重生成并发安全、SSRF 安全红线、工程约束三层门禁到架构债盘点的完整技术故事。如果从第一篇开始一路读到这里，你应该能理解：**v0.2 不是一个孤立的功能迭代，而是一次系统性的升级——把 WorkBuddy 从"只会说"的 RAG 问答助手，变成了"会动手"的本地优先 Agent 平台的第一块基石**。

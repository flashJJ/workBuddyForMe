# WorkBuddy For Me 一键开发脚本：环境检查 + 启动 Web 开发服务器
# 用法：powershell -File scripts/dev-web.ps1 [-Port 3000]
param(
    [int]$Port = 3000
)
...
Write-Host "环境检查通过（node $nodeVersion / pnpm $pnpmVersion），启动 Web 开发服务器：http://127.0.0.1:$Port" -ForegroundColor Green
Set-Location $repoRoot
pnpm --filter @wbfm/web dev
